<?php
/**
 * Title: Page type 4
 * Slug: ng1-base/template-page-type-4
  * Categories: pixelea, featured
 * Keywords: starter
 * Block Types: core/post-content
 * Post Types: page, wp_template
 * Viewport width: 1400
 */
?>
<!-- wp:pattern {"slug":"ng1-base/spacer-300"} /-->
<!-- wp:pattern {"slug":"ng1-base/heading-with-line"} /-->
<!-- wp:pattern {"slug":"ng1-base/spacer-040"} /-->
<!-- wp:pattern {"slug":"ng1-base/section-text-2cols"} /-->
<!-- wp:pattern {"slug":"ng1-base/spacer-120"} /-->
<!-- wp:pattern {"slug":"ng1-base/media-text-type-6-duotone"} /-->
 <!-- wp:pattern {"slug":"ng1-base/spacer-120"} /-->
<!-- wp:pattern {"slug":"ng1-base/section-information-2"} /-->
<!-- wp:pattern {"slug":"ng1-base/spacer-160"} /-->